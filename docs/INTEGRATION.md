# Meldlings — the one-line fix, plus Training & Gauntlet

## 1. Your vertical text — the actual line

I found it in your `scripts/main.gd`:
...
func _label(text: String, font_size: int, color: Color) -> Label:
	...
	label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART   # <-- delete this line
	return label
```

Every label in the game is built by `_label()`, so every label gets autowrap.
A Label with autowrap on reports a **minimum width of ~1 character**, so any
label sitting in an HBox where nothing claims the spare width collapses to ~10px
and wraps after every letter.

**Minimal fix — delete that line.** Labels then size to their text, which is what
you want everywhere in this UI.

If you want a couple of them to wrap (the log, the relic descriptions), do it at
the call site rather than in the factory:

```gdscript
var l := _label("...", 14, Color.WHITE)
l.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
l.size_flags_horizontal = Control.SIZE_EXPAND_FILL
l.custom_minimum_size.x = 240
```

Your `log_label` is a `RichTextLabel`, which is unaffected — it's already fine.

**Same class of bug on your card backs** (the hatched squares): a `TextureRect`
with `expand_mode = EXPAND_IGNORE_SIZE` has a minimum size of **zero**. Always
pair it with `custom_minimum_size = texture.get_size() * scale`.

---

## 2. Suit adapter — your cards now work with the card art

Your cards store suits as Unicode pips (`SUITS := ["♥", "♦", "♣", "♠"]`), mine
used letters. `CardArt` now speaks both, plus int indices:

```gdscript
CardArt.suit_letter("♥")     # -> "H"    (also takes "H", or 1)
CardArt.suit_symbol("H")     # -> "♥"
CardArt.face(7, "♥")         # -> the 7 of Hearts texture

CardArt.from_legacy({"rank": 7, "suit": "♥"})   # -> {"rank": 7, "suit": "H"}
CardArt.to_legacy({"rank": 7, "suit": "H"})     # -> {"rank": 7, "suit": "♥"}
CardArt.from_legacy_hand(player_hand)           # whole array
```

So you can drop `CardView` into your existing `hand_box` without touching your
deck code:
...
for card in player_hand:
	...
	v.setup(int(card.rank), card.suit)     # accepts "♥" directly
```

**Making your rules authoritative.** Your `RummyRules.classify()` returns an
`ActionType` int; mine returns a Dictionary. To have one source of truth, bridge
in `scripts/cards/meld_rules.gd`:

```gdscript
const _MAP := {
	RummyRules.ActionType.BRACE: Kind.PAIR,
	RummyRules.ActionType.PREP: Kind.RUN2,
	RummyRules.ActionType.STRIKE: Kind.RUN,
	RummyRules.ActionType.RALLY: Kind.SET,
	RummyRules.ActionType.GRAND_MELD: Kind.GRAND,
}

static func classify(cards: Array) -> Dictionary:
	var a := RummyRules.classify(CardArt.to_legacy_hand(cards))
	var kind: int = _MAP.get(a, Kind.NONE)
	return _result(kind, kind != Kind.NONE, RummyRules.action_name(a))
```

Keep `can_join()` on my side either way — it's UI logic, and it's what drives
the dimming hints.

**Save file collision, worth knowing:** your `main.gd` and my `SaveManager` both
write `user://meldlings_save.cfg` with `progress/essence` and
`progress/vitality`. Same keys, so they interoperate — but if you run both
entry points they'll share (and overwrite) progress. Rename one if that bothers you.

---

## 3. TRAINING (the sandbox)

Title → **TRAINING · sandbox** → pick a Meldling.

This is a **real battle** — `SandboxScreen extends BattleScreen`, so every button
exercises the shipping combat code, not a mock. Tap **⚙ TRAINING** (top right)
for the drawer:

| Section | What it does |
|---|---|
| Deal me a meld | One tap for Pair / 2-Run / 3-Run / 5-Run / 3-Set / GRAND |
| Exact card | Rank + suit pickers, straight into hand |
| Opponent | Swap enemy mid-fight; set foe to 10 HP / 1 HP / full |
| Resources | +Focus, +HP, draw 3, apply Burn / Hex / Thorns to yourself |
| Relics | Grant any relic instantly and watch the numbers move |
| Flow | Force win, force loss, restart |

Above all of it is a **live damage readout**: select cards and it shows what the
meld *would* do before you commit — `STRIKE 23 raw (18 after Block 5)`. That's
`Combat.preview_meld()`, and it's pure, so it can't desync the battle.

**Use it to answer the question that decides the game.** Give yourself a pair and
a partial run, then check whether bracing to build the longer run actually beats
playing the pair now. If patience never wins, the "master Rummy" pillar isn't
live yet and the numbers need changing. You can't learn that from random runs —
you have to set the position.

Training never ends the run; win or lose, it resets so you can keep testing.

---

## 4. GAUNTLET (the demo run)

Title → **GAUNTLET · demo run**.

Same combat, map, and relic code as a normal run — just a handcrafted, fully
deterministic 7-node sequence instead of a generated map:

```
Deadwood → Cache → Shuffler → Rest → Jokester (elite) → Cache → Kingpin (boss)
```

Roughly ten minutes, escalating, identical every time. That last part is the
point: when you show it to someone, you want to know exactly what they'll hit.

It's `RunState.GAUNTLET` — a list of dictionaries. Reorder it, swap enemies, add
nodes; no other code changes.

---

## 5. What I'd test first, in the sandbox

1. **Is BRACE-to-build ever correct?** The core question. If not, either Block
   needs to scale better or enemy damage needs to ramp slower.
2. **Do STRIKE and RALLY feel different**, or is one just better? Right now
   STRIKE is `5n + 3(n-3)` and RALLY is `4n + Burn`. At n=3 that's 15 vs 12+burn —
   close. At n=5 it's 31 vs 20+burn. Runs scale, sets don't. Check that reads.
3. **Is 2 Focus the right budget?** Try 1 and 3 with the +Focus button.
4. **Does the enemy intent give you enough time to react?**

Those four answers are worth more than any feature I could add next.
# Read this first
