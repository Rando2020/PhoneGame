...
## TUNED — see BALANCE.md. Measured with web/tune.js over ~100k simulated battles.
## A hand of 8 could produce a damage meld only 20.9% of the time; 10 gives 40.0%.
const BASE_HAND := 10
const BASE_FOCUS := 2
const ENEMY_DMG := 0.9

static func strike_damage(n: int) -> int: return 5 * n + 4 * (n - 3)
static func rally_damage(n: int) -> int: return 6 * n
static func brace_block(n: int) -> int: return 8 + n * 2
const PREP_DRAW := 3
...
func play_meld(cards: Array) -> Dictionary:
	...
	match verdict.kind:
		MeldRules.Kind.PAIR:
			var b: int = brace_block(n) + run.relic_value("brace_bonus")
			player.block += b
			result.block = b
		...
		MeldRules.Kind.RUN2:
			var extra: int = PREP_DRAW + run.relic_value("draw_bonus")
			draw_to_hand(extra)
			focus += 1
			result.offensive = false
		...
		MeldRules.Kind.RUN:
			var dmg: int = strike_damage(n) + run.relic_value("strike_bonus")
			result.damage = _attack(player, enemy, dmg)
			result.text = "STRIKE — %d damage." % result.damage

		MeldRules.Kind.SET:
			var dmg2: int = rally_damage(n)
...
## What WOULD this meld do? Pure -- mutates nothing. Drives the sandbox readout.
func preview_meld(cards: Array) -> String:
	var verdict := MeldRules.classify(cards)
	...
	match verdict.kind:
		MeldRules.Kind.PAIR:
			return "BRACE  +%d Block" % (brace_block(n) + run.relic_value("brace_bonus"))
		MeldRules.Kind.RUN2:
			return "PREP  draw %d, +1 Focus" % (PREP_DRAW + run.relic_value("draw_bonus"))
		MeldRules.Kind.RUN:
			var d: int = strike_damage(n) + run.relic_value("strike_bonus")
			return "STRIKE  %d raw  (%d after Block %d)" % [
				d, maxi(0, d - player.hex - enemy.block), enemy.block]
		MeldRules.Kind.SET:
			var d2: int = rally_damage(n)
			return "RALLY  %d raw + %d Burn" % [d2, 2 + run.relic_value("burn_bonus")]
		MeldRules.Kind.GRAND:
			return "GRAND MELD  %d raw, 3 Hex, 8 Block" % (26 + run.relic_value("strike_bonus"))
	return verdict.label


# ------------------------------------------------------------------ debug
## Sandbox only: put an exact card into hand.
func inject_card(rank: int, suit) -> Dictionary:
	var card := {"rank": clampi(rank, 1, 13), "suit": CardArt.suit_letter(suit)}
	hand.append(card)
	return card


## Sandbox only: swap the opponent mid-battle.
func set_enemy(enemy_id: String, hp_override: int = -1) -> void:
	var d := CreatureDB.get_data(enemy_id)
	var hp: int = hp_override if hp_override > 0 else int(d.get("hp", 44))
	enemy = Combatant.new(enemy_id, d.get("name", enemy_id), hp)
	over = false
	_roll_intent()
	enemy_changed.emit()
...
func _roll_intent() -> void:
	...
	match pick:
		"STRIKE":
			amount = int(round((6 + tier * 3 + turn) * ENEMY_DMG))
		"BRACE":
			amount = 6 + tier * 2
		"RALLY":
			amount = int(round((4 + tier * 2) * ENEMY_DMG))
		"HEX":
			amount = 2
