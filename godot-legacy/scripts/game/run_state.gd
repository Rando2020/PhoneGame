...
## Curated demo run: same combat and relic code, handcrafted 7-node sequence.
## Deterministic, ~10 minutes, escalating -- the version you show people.
const GAUNTLET := [
	{"type": NodeType.BATTLE, "enemy": "deadwood"},
	{"type": NodeType.TREASURE, "enemy": ""},
	{"type": NodeType.BATTLE, "enemy": "shuffler"},
	{"type": NodeType.REST, "enemy": ""},
	{"type": NodeType.ELITE, "enemy": "jokester"},
	{"type": NodeType.TREASURE, "enemy": ""},
	{"type": NodeType.BOSS, "enemy": "kingpin"},
]

var gauntlet: bool = false


func start_gauntlet(p_meldling: String) -> void:
	start(p_meldling)
	gauntlet = true
	map.clear()
	for i in GAUNTLET.size():
		var entry: Dictionary = GAUNTLET[i]
		map.append([{
			"type": entry["type"],
			"enemy": entry["enemy"],
			"seen": false,
			"links": [] if i == GAUNTLET.size() - 1 else [0],
		}])
