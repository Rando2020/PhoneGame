...
func _make(id: String) -> Screen:
	match id:
		...
		"sandbox": return SandboxScreen.new()
...
func start_gauntlet(meldling: String) -> void:
	run.start_gauntlet(meldling)
	goto("map")


## Training: seed a throwaway run so BattleScreen has something to render,
## then hand control to the sandbox.
func start_training(meldling: String = "pip", enemy: String = "deadwood") -> void:
	run.start(meldling)
	run.current_enemy = enemy
	goto("sandbox")
