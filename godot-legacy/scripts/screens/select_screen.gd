...
var mode: String = "run"      ## "run" | "gauntlet" | "training"
...
func build() -> void:
	...
	if mode != "run":
		col.add_child(UIKit.label(
			"GAUNTLET — curated 7-node demo" if mode == "gauntlet" else "TRAINING — sandbox",
			14, UIKit.GOLD, HORIZONTAL_ALIGNMENT_CENTER))
	...
	var go := UIKit.button(_mode_label(), 22, 60)
	go.pressed.connect(func():
		audio().sfx("meld_rally")
		match mode:
			"gauntlet": game.start_gauntlet(_chosen)
			"training": game.start_training(_chosen)
			_: game.start_run(_chosen))
	col.add_child(go)
...
func _mode_label() -> String:
	match mode:
		"gauntlet": return "RUN THE GAUNTLET"
		"training": return "ENTER TRAINING"
	return "START"
